...
class DBWrapper {
    ...
    private function lazy_instantiation() {
        if ($this->db === null) {
            $this->db = new SimpleMySQLi();
        }
    }

    public function query($stmt, $values = [], string $types = '') {
        $this->lazy_instantiation();
        error_log($stmt . ' % (' . implode(',', array_values($values)) . ')');
        return $this->db->query($stmt, $values, $types);
    }
...
/**
 * Class SimpleMySQLi
 * See https://github.com/WebsiteBeaver/Simple-MySQLi
 * Made modifications to suit the application
 * @version 1.5.5
 */
class SimpleMySQLi {
	private $mysqli = null;
	 ...
	 * @throws mysqli_sql_exception If any mysqli function failed due to mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT)
	 */
	public function query(string $sql, $values = [], string $types = '') {
